...
└── skills/
    ...
    ├── reviewing-plans/    ← ревьюит план: слабые места, риски, пробелы, оверинжиниринг
    └── executing-plans/    ← исполняет план, стоп при блокере, чекпоинты
```

**Цепочка:** `brainstorming` → `writing-plans` → `reviewing-plans` → `executing-plans`
...
- `reviewing-plans`: критически ревьюит план до старта — покрытие спека, плейсхолдеры, тех. риски, тесты, порядок зависимостей, консистентность типов/имён, YAGNI. Выводит таблицу находок в чат; при Blocker-ах предлагает исправить план на месте.
...
**Установка:** скопировать все четыре папки в `.claude/skills/` проекта.

---

### `swagger/` — генерация OpenAPI/Swagger для модуля Node.js

```
swagger/
└── skills/
    ├── swagger-jsdoc/   ← inline @swagger JSDoc-блоки прямо в коде (формат swagger-jsdoc)
    ├── swagger-json/    ← отдельный файл-спека <модуль>.swagger.json
    ├── swagger-yml/     ← отдельный файл-спека <модуль>.swagger.yml
    └── swagger-verify/  ← верификация готовой спеки: валидность OpenAPI + сверка с кодом
```

**Как работает:** генераторы (`swagger-yml`/`swagger-json`/`swagger-jsdoc`) на входе берут **путь к модулю** и
вытаскивают контракт API **строго из кода** (роуты, path/query/body/headers, коды и тела ответов, модели из
ORM/валидаторов/TS-типов, auth-middleware → `security`). Главный принцип — **не выдумывать**: описываются только
роуты, которые есть в коде, незнакомые типы помечаются плейсхолдером `TODO: уточнить`. `swagger-yml`/`swagger-json`
отличаются только форматом вывода (шаги разбора идентичны); `swagger-jsdoc` не создаёт файл, а встраивает блоки
`/** @swagger ... */` над обработчиками. `swagger-verify` — парная проверка: берёт **готовый** артефакт (yml/json/inline)
и сверяет его (а) на формальную валидность OpenAPI 3.0 и (б) с кодом — нет ли выдуманных/потерянных роутов,
совпадают ли параметры/ответы/модели; отчёт таблицей в чат, файлы не правит. Валидация артефакта — `npx @redocly/cli lint`.

> Заменяет старый `swagger-gen` (более гранулярно: выбор формата + inline-вариант).
> Не для дизайна нового API (→ `api-designer`) и не для ревью согласованности контракта (→ агент `code-review-contracts`).

**Установка:** скопировать нужную папку (или все четыре) в `.claude/skills/` проекта. Скиллы независимы.

---

### `playwright/` — генерация E2E-тестов Playwright по компоненту

```
playwright/
└── skills/
    └── playwright-e2e/   ← генерит describe.only-блок E2E для React-компонента по пути к файлу
```

**Как работает:** на входе — **путь к компоненту/файлу** (+ опционально сценарии словами, включая «проверь
разные комбинации»). Скилл собирает элементы по тест-атрибутам (в DOM все — `data-test-id`; React-проп `testId`
тоже становится `data-test-id`), выводит кандидатные сценарии **строго из кода** (обработчики, состояния,
условный рендер, комбинации пропсов), **показывает список сценариев в чат и ждёт подтверждения** (HARD-GATE),
затем выдаёт **ровно один** блок `test.describe.only("<что проверяем>", () => { ... })` через `getByTestId()`.
Главный принцип — **не выдумывать**: только то, что есть в коде; незнакомое (роут, ожидаемые значения) →
`// TODO`. Допущения (не проверяет и не ищет): на всех элементах есть `data-test-id`, Playwright настроен,
навигация в `beforeEach` — поэтому не читает `playwright.config`, не добавляет импорты/шапку, не ищет роуты и
не пересказывает файл. Блок рассчитан на **вставку в уже настроенный Playwright-спек**. Артефакт по умолчанию —
`.gigacode/playwright/<component-name>/<component-name>.spec.ts` (только сам блок, без импортов).

> Не для настройки Playwright с нуля, не для component-тестов в изоляции (`@playwright/experimental-ct-react`),
> не для unit-тестов логики (→ Jest/Vitest), не для ревью существующих тестов.

**Установка:** скопировать папку в `.claude/skills/` проекта. Скилл независим.
