...
playwright/
└── skills/
    └── playwright-e2e/   ← генерит *.spec.ts для React-компонента по пути к файлу
```

**Как работает:** на входе — **путь к компоненту/файлу** (+ опционально сценарии словами, включая «проверь
разные комбинации»). Скилл собирает элементы по тест-атрибутам (`data-test-id` / `data-testid` / `testId`),
выводит кандидатные сценарии **строго из кода** (обработчики, состояния, условный рендер, комбинации пропсов),
**показывает список сценариев в чат и ждёт подтверждения** (HARD-GATE), затем пишет E2E-тесты через
`getByTestId()`. Роут выводит best-effort, иначе оставляет `// TODO: роут`. Главный принцип — **не выдумывать**:
только то, что есть в коде; незнакомое помечается `// TODO`. Тесты рассчитаны на **копипаст в уже настроенный
Playwright** — скилл ничего не запускает, не правит `playwright.config` и не поднимает сервер. Артефакты по
умолчанию — `.gigacode/playwright/<component-name>/<component-name>.spec.ts`.
