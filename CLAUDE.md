...
**Как работает:** на входе — **путь к модулю**. Скилл вытаскивает контракт API **строго из кода**
(роуты, path/query/body/headers, коды и тела ответов, модели из ORM/валидаторов/TS-типов, auth-middleware →
`security`). Главный принцип — **не выдумывать**: описываются только роуты, которые есть в коде, незнакомые
типы помечаются плейсхолдером `TODO: уточнить`. `swagger-yml`/`swagger-json` отличаются только форматом
вывода (шаги разбора идентичны); `swagger-jsdoc` не создаёт файл, а встраивает блоки `/** @swagger ... */`
над обработчиками. Валидация артефакта — `npx @redocly/cli lint`.
...
**Установка:** скопировать нужную папку (или все три) в `.claude/skills/` проекта. Скиллы независимы.
