...
Каждый пишет свой файл-отчёт сам. Суб-скиллы вызываются **по имени через инструмент `Skill`** —
не читай их SKILL.md вручную и не ищи их по путям на диске.
...
В промпте каждого агента укажи:
```
Вызови скилл `<aspect>` через инструмент Skill и передай ему:
- вход: <diff/путь>
- путь для отчёта: <AGENT_HOME>/reports/<YYYY.MM.DD>/code-review-<aspect>.md
- язык отчёта: <язык>
- <дополнительные override'ы пользователя, если есть>
```
...
1. Вызови скилл `code-review-<aspect>` через инструмент `Skill`, передав вход, путь для отчёта и язык.
2. Дождись завершения, затем переходи к следующему аспекту.
